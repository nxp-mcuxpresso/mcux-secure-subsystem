...
status_t sss_ecdh_calc_secret(sss_ecdh_context_t *pEcdh_ctx, unsigned char *wrk_buf, size_t wrk_buf_lg)
{
    ...
    do
    {
        ...
        if ((ret = SSS_KEY_STORE_SET_KEY(&pEcdh_ctx->peerPublicKey, (const uint8_t *)wrk_buf, key_sz, coordinateBitsLen,
        ...
        if ((ret = sss_sscp_asymmetric_dh_derive_key(&dCtx, &pEcdh_ctx->peerPublicKey, &pEcdh_ctx->sharedSecret)) !=
            kStatus_SSS_Success)
        {
            break;
        }
        if ((ret = sss_sscp_key_store_get_key(&g_keyStore, &pEcdh_ctx->sharedSecret, wrk_buf, &coordinateLen,
                                              &coordinateBitsLen, (sss_key_part_t)NULL)) != kStatus_SSS_Success)
