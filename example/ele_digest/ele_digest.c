...
status_t test_sha_context_switching(void)
{
    ...
    do
    {
        ...
        status = sss_sscp_digest_update(&ctx_sha1, (uint8_t *)&message[example_hash_update_size],
                                        example_hash_update_size);
        if (status != kStatus_SSS_Success)
        {
            break;
...
int main(void)
{
    ...
    /*
     * This code example demonstrates EdgeLock usage of one-go and multi-part hashing operations via SSSAPI.
